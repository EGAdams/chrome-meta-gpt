...
t_dict = {"Required Swift third-party packages": "\"\"\"\nflask==1.1.2\npygame==2.0.1\n\"\"\"\n",
          ...
          "Logic Analysis": [
              ...
              ["static/js/script.js",
                  "Handles user interactions and updates the game UI."],
...
def test_create_model_class():
    test_class = ActionOutput.create_model_class(
        "test_class", WRITE_TASKS_OUTPUT_MAPPING)
    assert test_class.__name__ == "test_class"


def test_create_model_class_with_mapping():
    t = ActionOutput.create_model_class(
        "test_class_1", WRITE_TASKS_OUTPUT_MAPPING)
    t1 = t(**t_dict)
    value = t1.dict()["Task list"]
    assert value == ["game.py", "app.py", "static/css/styles.css",
